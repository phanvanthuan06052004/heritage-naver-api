...
export const analyzeSummary = (results) => {
  ...
  const summary = {
    ...
    avgBLEU:
      calculateAverage(results, "metrics.bleu") ||
      calculateAverageNested(results, "bleu"),
...
export const printSummary = (results) => {
  ...
  console.log(
    `   Average Sources Retrieved: ${summary.avgSourcesRetrieved.toFixed(2)}`
  );

  console.log("\n📈 Average Metrics:");
  console.log(
    `   BLEU Score:            ${summary.avgBLEU.toFixed(4)} ${getScoreEmoji(
      summary.avgBLEU
    )}`
  );
  console.log(
    `   ROUGE-L F1:            ${summary.avgROUGEL.toFixed(4)} ${getScoreEmoji(
      summary.avgROUGEL
    )}`
  );
  console.log(
    `   Cosine (TF-IDF):       ${summary.avgCosineTFIDF.toFixed(
      4
    )} ${getScoreEmoji(summary.avgCosineTFIDF)}`
  );

  if (summary.avgCosineSemantic > 0) {
    console.log(
      `   Cosine (Semantic):     ${summary.avgCosineSemantic.toFixed(
        4
      )} ${getScoreEmoji(summary.avgCosineSemantic)}`
    );
  }
  ...
  Object.entries(summary.qualityDistribution).forEach(([quality, count]) => {
    if (count > 0) {
      ...
      console.log(
        `   ${quality.padEnd(12)} ${count
          .toString()
          .padStart(3)} (${percentage.padStart(5)}%) ${bar}`
      );
    }
  });
...
export const printWorstPerformers = (results, count = 5) => {
  ...
  const resultsWithScore = results
    ...
    .map((r) => ({
      ...
      compositeScore:
        ((r.metrics.bleu || 0) +
          (r.metrics.rouge_l_f1 || 0) +
          (r.metrics.cosine_tfidf || 0) +
          (r.metrics.cosine_semantic || r.metrics.cosine_tfidf || 0)) /
        4,
    }))
  ...
  resultsWithScore.forEach((result, index) => {
    console.log(
      ...
      } (Score: ${result.compositeScore.toFixed(4)})`
    );
...
export const printDetailedResults = (results) => {
  ...
  const rows = results
    ...
    .map((r) => [
      ...
      (r.metrics.bleu || 0).toFixed(3),
      (r.metrics.rouge_l_f1 || 0).toFixed(3),
      (r.metrics.cosine_tfidf || 0).toFixed(3),
      classifyQuality(r.metrics),
      (r.metrics.totalTime || r.execution_time_ms || 0).toFixed(0),
    ]);
...
export const generateSuggestions = (results) => {
  ...
  if (avgSources < 2) {
    suggestions.push({
      ...
      suggestion:
        "Consider increasing topK parameter or improving document indexing",
    });
  }
  ...
  if (summary.avgBLEU < 0.3) {
    suggestions.push({
      ...
      suggestion:
        "Review prompt engineering or consider fine-tuning the generation model",
    });
  }
  ...
  if (summary.avgROUGEL < 0.4) {
    suggestions.push({
      ...
      suggestion:
        "Generated answers may be too different from expected format. Review context quality.",
    });
  }
  ...
  if (summary.avgCosineTFIDF < 0.5) {
    suggestions.push({
      ...
      suggestion:
        "Retrieved context may not be relevant. Improve embedding quality or document preprocessing.",
    });
  }
  ...
  const poorQuality =
    (summary.qualityDistribution.Poor || 0) +
    (summary.qualityDistribution["Very Poor"] || 0);
  ...
  if (poorPercentage > 30) {
    suggestions.push({
      ...
      suggestion:
        "Consider comprehensive review of RAG pipeline: document quality, retrieval strategy, and generation prompts",
    });
  }
  ...
  if (summary.failedQuestions > 0) {
    suggestions.push({
      ...
      suggestion:
        "Review error logs and improve error handling. Check API connectivity and rate limits.",
    });
  }
...
const calculateAverageNested = (results, key, isArray = false) => {
  const values = results
    ...
    .map((r) =>
      isArray && Array.isArray(r[key]) ? r[key].length : r.metrics[key]
    );
...
const diagnoseIssues = (result) => {
  ...
  if (result.sources && result.sources.length > 0) {
    const avgRelevance =
      result.sources.reduce((sum, s) => sum + (s.relevanceScore || 0), 0) /
      result.sources.length;
    if (avgRelevance < 0.5) {
      issues.push(
        "Low relevance scores - retrieved documents may not be relevant"
      );
    }
  } else {
  ...
  const generatedAnswer =
    result.generatedAnswer || result.generated_answer || "";
  ...
  if (result.metrics) {
    const allLow =
      result.metrics.bleu < 0.2 &&
      result.metrics.rouge_l_f1 < 0.2 &&
      result.metrics.cosine_tfidf < 0.3;
    if (allLow) {
      issues.push(
        "All metrics are low - fundamental mismatch between generated and expected"
      );
    }
  }
