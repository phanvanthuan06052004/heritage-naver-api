...
const uploadText = async (req, res, next) => {
  const schema = Joi.object({
    text: Joi.string().required().min(10).max(1000000).trim().messages({
...
const query = async (req, res, next) => {
  const schema = Joi.object({
    question: Joi.string().required().min(1).max(1000).trim().messages({
    ...
    heritageId: Joi.string()
      ...
      .messages({
        "string.pattern.base":
          "heritageId must be a valid MongoDB ObjectId (24 hex characters)",
      }),

    topK: Joi.number().integer().min(1).max(20).optional().default(5).messages({
